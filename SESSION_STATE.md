...
最新补充：会员套餐价格调整为月卡 300 元、季卡 500 元、年卡 800 元；试用次数用完或会员过期后，桌面端会自动弹出充值窗口；管理后台订单金额和套餐价格按“元”展示，订单状态和支付方式展示为中文；桌面端任务配置标题行右侧新增“注意事项”和“增加微信加人窗口”小按钮入口；点击开始任务前会弹出自动化接管鼠标键盘提示，确认后倒计时 5 秒再启动；打招呼语下方新增 3 条默认话术快捷选择；“创建标签”入口暂时从任务配置 UI 隐藏；用户教程页已更新为图文三步教程并引用任务配置截图，每日限额步骤强化新号/中期号/老号建议。
...
- 任务配置标题行右侧新增两个小按钮：“注意事项”跳转到左侧“用户教程”页面，“增加微信加人窗口”打开充值弹窗。
- 点击“开始任务”先弹出自动化提示，说明将控制鼠标键盘、需打开微信主窗口、确认后有 5 秒切换时间；倒计时结束后才执行原启动流程。
- 打招呼语输入框下方新增 3 条默认话术按钮，点击后填入当前 slot 的 greetingText，运行中禁用。
- “创建标签”暂不在任务配置 UI 中显示，底层 createTag 字段和启动参数保留，后续可恢复入口。
- 任务标题显示为 `微信1任务配置`、`微信2任务配置`、`微信3任务配置`。

### 用户教程

- `desktop/src/MainPage.tsx`：教程页改为图文三步教程，说明每日限额、默认招呼语、开始任务。
- 每日限额步骤新增账号阶段建议：新号（0–3 个月，未养好）、中期号（3 个月–1 年，实名绑卡）、老号（1 年以上、高活跃、无违规），并突出新号每小时最多加 2 人、单次间隔 ≥ 50 分钟。
- `desktop/public/tutorial/`：新增三张教程截图，分别用于限额、招呼语、开始任务步骤。
- 根据会员套餐渲染 1/2/3 张任务卡。
- 每个任务卡独立持久化配置。
- 启动前读取对应 slot 的微信绑定。
...
- 本轮调整任务配置区入口 UI 后已执行 `npm run build` in `desktop`
- 本轮调整任务配置区入口 UI 后已执行 `npm run lint` in `desktop`
- 本轮新增开始任务前自动化提示弹窗后已执行 `npm run build` in `desktop`
- 本轮新增开始任务前自动化提示弹窗后已执行 `npm run lint` in `desktop`
